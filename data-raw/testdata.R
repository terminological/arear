...
.repeated = function(shape, offset_x, offset_y, xlim, ylim) {
  len = dim(shape)[1]
  right = array(
    c(rep(offset_x[1], len), rep(offset_x[2], len)),
    dim = c(len, 2)
  )
  up = array(c(rep(offset_y[1], len), rep(offset_y[2], len)), dim = c(len, 2))
  grid = tibble(x = min(xlim):max(xlim)) %>%
    tidyr::crossing(tibble(y = min(ylim):max(ylim)))
  grid = grid %>%
...
demand = bind_rows(
  ...
  demand %>%
    mutate(demand = ifelse((x + y) %% 2 == 0, 1, 0), type = "chessboard")
)
...
supply = tibble::tibble(
  ...
  y = c(-5, -5, 5),
  supply = c(1, 3, 2),
  id = c("bottom_left", "bottom_right", "top_right")
) %>%
  sf::st_as_sf(coords = c("x", "y"), crs = 4326)
ggplot(grid2) +
  geom_sf(aes(fill = demand)) +
  geom_sf(data = supply, aes(size = supply), colour = "red")
...
supply4 = tibble::tibble(
  x = c(-5, 5, 5, -4.9, -5),
  y = c(-5, -5, 5, -4.9, 5),
  supply = c(1, 3, 2, 2, 1),
  id = c("one", "two", "three", "four", "four")
) %>%
  sf::st_as_sf(coords = c("x", "y"), crs = 4326)

offset_box = sf::st_as_sf(
  tibble::tibble(
    name = "offset box",
    geometry = list(sf::st_polygon(list(square * 2.5 + 0.5)))
  ),
  crs = 4326
)
